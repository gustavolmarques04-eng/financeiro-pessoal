...
from core.models import Category, ExpenseInstallment
from core.utils import split_installments, to_cents

from .conftest import NOVEMBRO, OUTUBRO, SETEMBRO
...
def test_compra_parcelada_espalha_pelos_meses(session: Session, gasto) -> None:
    """R$ 600 em 6x a partir de setembro gera R$ 100 por mês."""
    compra = gasto(600.00, Category.COMPRAS, mes=SETEMBRO, parcelas=6)
    ...
    assert repo.sum_expenses(session, SETEMBRO) == to_cents(100)
    assert repo.sum_expenses(session, date(2027, 2, 1)) == to_cents(100)


def test_gasto_a_vista_cai_no_mes_da_compra(session: Session, gasto) -> None:
    """Sem parcelamento, o gasto inteiro pertence ao mês da compra."""
    gasto(250.00, Category.NAMORADA, mes=SETEMBRO)
    assert repo.sum_expenses(session, SETEMBRO) == to_cents(250)
    assert repo.sum_expenses(session, OUTUBRO) == 0


def test_editar_compra_parcelada_recalcula_parcelas(session: Session, gasto) -> None:
    ...
    assert repo.sum_expenses(session, date(2027, 2, 1)) == 0


# --------------------------------------------------------------------------
# 6 — exclusão em cascata
# --------------------------------------------------------------------------
def test_excluir_compra_remove_todas_as_parcelas(session: Session, gasto) -> None:
    """Apagar a compra não deixa parcelas órfãs."""
    compra = gasto(600.00, Category.VIAGEM, mes=SETEMBRO, parcelas=6)
    assert session.scalars(select(ExpenseInstallment)).all()

    repo.delete_expense(session, compra.id)

    assert session.scalars(select(ExpenseInstallment)).all() == []
    assert repo.sum_expenses(session, SETEMBRO) == 0
    assert repo.sum_expenses(session, OUTUBRO) == 0
...
# --------------------------------------------------------------------------
def test_gasto_reduz_apenas_a_sua_categoria(session: Session, receita, gasto) -> None:
    ...
    amigos = next(g for g in plano.gastos if g.categoria is Category.AMIGOS)
...
def test_estouro_de_orcamento_fica_negativo(session: Session, receita, gasto) -> None:
    """Gastar mais que o orçamento deixa o disponível negativo."""
    receita(1000.00)
    gasto(200.00, Category.AMIGOS, mes=SETEMBRO)

    linha = next(
        g for g in budget.get_month_plan(session, SETEMBRO).gastos
        if g.categoria is Category.AMIGOS
    )
...
def test_categorias_mensais_nao_acumulam(session: Session, receita, gasto) -> None:
    """Namorada recomeça do zero no mês seguinte."""
    receita(1000.00, mes=SETEMBRO)
    receita(1000.00, mes=OUTUBRO)
    gasto(50.00, Category.NAMORADA, mes=SETEMBRO)

    setembro = next(
        g for g in budget.get_month_plan(session, SETEMBRO).gastos
        if g.categoria is Category.NAMORADA
    )
    outubro = next(
...
def test_envelope_de_compras_acumula_entre_meses(
    ...
    assert budget.saldo_compras(session, SETEMBRO) == to_cents(5.54) + to_cents(265.70)

    receita(3000.00, mes=OUTUBRO)
    budget.confirmar_separacao(session, OUTUBRO, Category.COMPRAS)
    saldo_outubro = budget.saldo_compras(session, OUTUBRO)
    assert saldo_outubro == to_cents(5.54) + to_cents(265.70) + to_cents(270)

    gasto(500.00, Category.COMPRAS, mes=OUTUBRO)
    assert budget.saldo_compras(session, OUTUBRO) == saldo_outubro - to_cents(500)
    assert budget.saldo_compras(session, SETEMBRO) == to_cents(5.54) + to_cents(265.70)


def test_envelope_usa_separado_e_nao_planejado(session: Session, receita) -> None:
    """Sem confirmar a separação, o envelope não cresce."""
    receita(2952.21, mes=SETEMBRO)
    assert budget.saldo_compras(session, SETEMBRO) == 0

    budget.confirmar_separacao(session, SETEMBRO, Category.COMPRAS)
    assert budget.saldo_compras(session, SETEMBRO) == to_cents(265.70)


def test_envelope_de_viagem_acumula(session: Session, receita, gasto) -> None:
    ...
    budget.confirmar_separacao(session, OUTUBRO, Category.VIAGEM)

    assert budget.saldo_viagem(session, OUTUBRO) == to_cents(413.31) * 2

    gasto(1000.00, Category.VIAGEM, mes=NOVEMBRO)
    assert budget.saldo_viagem(session, NOVEMBRO) == to_cents(413.31) * 2 - to_cents(1000)


def test_parcelas_futuras_listam_compromissos(session: Session, gasto) -> None:
    """Parcelas depois do mês selecionado aparecem agrupadas por mês."""
    gasto(300.00, Category.COMPRAS, mes=SETEMBRO, parcelas=3)
